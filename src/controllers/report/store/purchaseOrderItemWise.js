// reports/purchaseOrderItemWise.js
// Item-wise grouped PurchaseOrder report.
// Same data source as purchaseOrderDetails, regrouped by ItemCode.
...
function buildDocDefinition(rows, companyName, fromDate, toDate) {
  ...
  // Group rows by ItemCode (fallback to ItemName), sorted by item name
  ...
  // Column widths (14 columns): S.No, Date, OrderNo, Supplier, Dept, Qty, Rate, Amount, Discount, P&F, Other, GST, RND, NetAmount
  const widths = [22, 50, 40, '*', '*', 40, 44, 50, 44, 40, 44, 50, 32, 52];

  const headerFill = '#1A3C7B';
  const headerText = '#FFFFFF';
  const itemFill = '#E8F0FE';
  const itemText = '#1A3C7B';
  ...
  const headerRow = [
    'S.No', 'Date', 'Order No', 'Supplier Name', 'Department',
  ...
  for (const key of sortedKeys) {
    ...
    body.push([
      {
        text: itemHeading,
        colSpan: 14, bold: true, color: itemText, fillColor: itemFill, fontSize: 9, margin: [2, 2, 0, 2]
      },
      {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
    ]);
  ...
  const summaryNodes = buildSummaryPage({
    ...
    title: 'PURCHASE ORDER SUMMARY - ITEM WISE',
  ...
  return {
    ...
    content: [
      ...summaryNodes,
      {
        stack: [
          { text: companyName, alignment: 'center', fontSize: 16, bold: true, color: '#7B3F00', margin: [0, 0, 0, 6] },
          { text: 'PURCHASE ORDER - ITEM WISE', alignment: 'center', fontSize: 12, bold: true, color: '#008000', margin: [0, 0, 0, 6] },
          { text: `From : ${ddmmyyyy(fromDate)}   To : ${ddmmyyyy(toDate)}`, alignment: 'center', fontSize: 10, bold: true, margin: [0, 0, 0, 10] }
        ],
        pageBreak: 'before'
      },
