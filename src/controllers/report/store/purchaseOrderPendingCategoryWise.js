// reports/purchaseOrderPendingCategoryWise.js
// Category-wise grouped Purchase Order PENDING report.
// SP: sp_RptPurchaseOrderDetailsPending (CompanyCode, FromDate, ToDate)
// Grouped by ItemCategoryCode. Columns: PO No, Date, Mode, Supplier, Item, UOM,
//   Order Qty (POQty), Recei. Qty (PORQty), Pend. Qty (PendingQty), Rate, Net Amount (PendingNetAmount)
...
function buildDocDefinition(rows, companyName, fromDate, toDate) {
  ...
  // 11 columns: S.No, PO No, Date, Mode, Supplier, Item, UOM, Order Qty, Recv Qty, Pend Qty, Rate, Net Amount
  ...
  const headerRow = [
    'S.No', 'PO No', 'Date', 'Mode', 'Supplier Name', 'Item Name', 'UOM',
  ...
  const summaryNodes = buildSummaryPage({
    ...
    title: 'PURCHASE ORDER PENDING SUMMARY - CATEGORY WISE',
  ...
  return {
    ...
    content: [
      ...summaryNodes,
      {
        stack: [
          { text: companyName, alignment: 'center', fontSize: 16, bold: true, color: '#7B3F00', margin: [0, 0, 0, 6] },
          { text: 'INWARD PENDING - CATEGORY WISE', alignment: 'center', fontSize: 12, bold: true, color: '#008000', margin: [0, 0, 0, 6] },
