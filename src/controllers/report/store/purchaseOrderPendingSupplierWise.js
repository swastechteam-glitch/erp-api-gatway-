// reports/purchaseOrderPendingSupplierWise.js
// Supplier-wise grouped Purchase Order PENDING report.
// SP: sp_RptPurchaseOrderDetailsPending (CompanyCode, FromDate, ToDate)
// Grouped by SupplierCode.
...
function buildDocDefinition(rows, companyName, fromDate, toDate) {
  ...
  for (const r of rows) {
    const k = (r.SupplierCode != null ? String(r.SupplierCode) : '') + '||' + (str(r, 'SupplierName') || '(Unknown Supplier)');
  ...
  // 11 columns: S.No, PO No, Date, Mode, Department, Item, UOM, Order Qty, Recv Qty, Pend Qty, Rate, Net Amount
  ...
  const headerRow = [
    'S.No', 'PO No', 'Date', 'Mode', 'Department', 'Item Name', 'UOM',
  ...
  for (const key of sortedKeys) {
    ...
    const supplierName = key.split('||')[1] || '(Unknown Supplier)';
    ...
    for (const r of group) {
      ...
      const maxLines = Math.max(deptLines, itemLines, 1);
  ...
  const summaryNodes = buildSummaryPage({
    ...
    title: 'PURCHASE ORDER PENDING SUMMARY - SUPPLIER WISE',
  ...
  return {
    ...
    content: [
      ...summaryNodes,
      {
        stack: [
          { text: companyName, alignment: 'center', fontSize: 16, bold: true, color: '#7B3F00', margin: [0, 0, 0, 6] },
          { text: 'INWARD PENDING - SUPPLIER WISE', alignment: 'center', fontSize: 12, bold: true, color: '#008000', margin: [0, 0, 0, 6] },
