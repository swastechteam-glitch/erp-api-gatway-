// reports/purchaseOrderCategoryWise.js
// Category-wise grouped PurchaseOrder report.
// Same data source as purchaseOrderDetails, regrouped by ItemCategoryCode.
...
function buildDocDefinition(rows, companyName, fromDate, toDate) {
  ...
  // Group rows by ItemCategoryCode (fallback to ItemCategoryName)
  const groupsMap = new Map();
  for (const r of rows) {
    const k = (r.ItemCategoryCode != null ? String(r.ItemCategoryCode) : '') + '||' + (str(r, 'ItemCategoryName') || '(Unknown Category)');
  ...
  const categoryFill = '#E8F0FE';
  const categoryText = '#1A3C7B';
  ...
  for (const key of sortedKeys) {
    ...
    const categoryName = key.split('||')[1] || '(Unknown Category)';

    body.push([
      {
        text: 'Category : ' + categoryName,
        colSpan: 14, bold: true, color: categoryText, fillColor: categoryFill, fontSize: 9, margin: [2, 2, 0, 2]
      },
      {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
    ]);
  ...
  const summaryNodes = buildSummaryPage({
    ...
    title: 'PURCHASE ORDER SUMMARY - CATEGORY WISE',
  ...
  return {
    ...
    content: [
      ...summaryNodes,
      {
        stack: [
          { text: companyName, alignment: 'center', fontSize: 16, bold: true, color: '#7B3F00', margin: [0, 0, 0, 6] },
          { text: 'PURCHASE ORDER - CATEGORY WISE', alignment: 'center', fontSize: 12, bold: true, color: '#008000', margin: [0, 0, 0, 6] },
          { text: `From : ${ddmmyyyy(fromDate)}   To : ${ddmmyyyy(toDate)}`, alignment: 'center', fontSize: 10, bold: true, margin: [0, 0, 0, 10] }
        ],
        pageBreak: 'before'
      },
