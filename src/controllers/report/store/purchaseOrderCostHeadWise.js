// reports/purchaseOrderCostHeadWise.js
// CostHead-wise grouped PurchaseOrder report.
// Same data source as purchaseOrderDetails, regrouped by CostHeadCode.
...
function buildDocDefinition(rows, companyName, fromDate, toDate) {
  ...
  // Group rows by CostHeadCode (fallback to CostHeadName)
  const groupsMap = new Map();
  for (const r of rows) {
    const k = (r.CostHeadCode != null ? String(r.CostHeadCode) : '') + '||' + (str(r, 'CostHeadName') || '(Unknown CostHead)');
  ...
  // Column widths (14 columns): S.No, Date, OrderNo, Item, UOM, Qty, Rate, Amount, Discount, P&F, Other, GST, RND, NetAmount
  const widths = [22, 50, 40, '*', 36, 40, 44, 50, 44, 40, 44, 50, 32, 52];

  const headerFill = '#1A3C7B';
  const headerText = '#FFFFFF';
  const costHeadFill = '#E8F0FE';
  const costHeadText = '#1A3C7B';
  ...
  const headerRow = [
    'S.No', 'Date', 'Order No', 'Item Name', 'UOM',
  ...
  for (const key of sortedKeys) {
    ...
    const costHeadName = key.split('||')[1] || '(Unknown CostHead)';

    body.push([
      {
        text: 'Cost Head : ' + costHeadName,
        colSpan: 14, bold: true, color: costHeadText, fillColor: costHeadFill, fontSize: 9, margin: [2, 2, 0, 2]
      },
      {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
    ]);
    ...
    for (const r of group) {
      ...
      const itemLines = estimateLines(itemName, 30);
  ...
  const summaryNodes = buildSummaryPage({
    ...
    title: 'PURCHASE ORDER SUMMARY - COST HEAD WISE',
    groupHeader: 'Cost Head',
    groupSummaries,
  ...
  return {
    ...
    content: [
      ...summaryNodes,
      {
        stack: [
          { text: companyName, alignment: 'center', fontSize: 16, bold: true, color: '#7B3F00', margin: [0, 0, 0, 6] },
          { text: 'PURCHASE ORDER - COST HEAD WISE', alignment: 'center', fontSize: 12, bold: true, color: '#008000', margin: [0, 0, 0, 6] },
          { text: `From : ${ddmmyyyy(fromDate)}   To : ${ddmmyyyy(toDate)}`, alignment: 'center', fontSize: 10, bold: true, margin: [0, 0, 0, 10] }
        ],
        pageBreak: 'before'
      },
