// Finisher Drawing Stoppage Wise report.
// Mirrors 04rptFinisherDrawingProductionStoppage.rdlc — one row per stoppage reason,
// with HRS and % columns for each shift, plus on-date and upto-date totals.
//
// SP: sp_Prodn_FinisherDrawing_Stoppage (CompanyCode, FromDate, ToDate)
...
const TITLE = 'FINISHER DRAWING STOPPAGE REPORT';
const FILE_NAME = 'FinisherDrawingProduction_StoppageWise';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'FINISHER DRAWING STOPPAGE - SUMMARY',
...
export const finisherDrawingStoppageReport = (req, res) => {
  return runReport(req, res, {
    spName: 'sp_Prodn_FinisherDrawing_Stoppage',
    fileName: FILE_NAME,
    buildDocDefinition
  });
};
