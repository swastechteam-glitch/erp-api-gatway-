...
// Mirrors 05rptComberEmployeePerformance.rdlc — one row per employee/date with
// production, efficiency, utilisation, index and grade; grouped by employee.
//
// SP: sp_Prodn_Comber_EmployeePerformance (CompanyCode, FromDate, ToDate)
...
const TITLE = 'COMBER EMPLOYEE PERFORMANCES REPORT';
const FILE_NAME = 'ComberProduction_EmployeePerformance';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const sorted = [...rows].sort((a, b) => {
    ...
    return new Date(a.ComberDate) - new Date(b.ComberDate);
  });
  ...
  for (const r of sorted) {
    ...
    body.push([
      cell(ddmmyyyy(r.ComberDate), 'center'),
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'COMBER EMPLOYEE PERFORMANCE - SUMMARY',
...
export const comberEmployeePerformanceReport = (req, res) => {
  return runReport(req, res, {
    spName: 'sp_Prodn_Comber_EmployeePerformance',
    fileName: FILE_NAME,
    buildDocDefinition
  });
};
