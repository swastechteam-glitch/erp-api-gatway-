// Simplex Stoppage Wise report.
// Mirrors 04rptSimplexProductionStoppage.rdlc — one row per stoppage reason, with
// HRS and % columns for each shift, plus on-date and upto-date totals.
//
// SP: sp_Prodn_Simplex_Stoppage (CompanyCode, FromDate, ToDate)
...
const TITLE = 'SIMPLEX STOPPAGE REPORT';
const FILE_NAME = 'SimplexProduction_StoppageWise';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'SIMPLEX STOPPAGE - SUMMARY',
...
export const simplexStoppageReport = (req, res) => {
  return runReport(req, res, {
    spName: 'sp_Prodn_Simplex_Stoppage',
    fileName: FILE_NAME,
    buildDocDefinition
  });
};
