// Comber Production Summary (Date Wise) report.
// Mirrors rptComberProductionSheetDateWise.rdlc — one row per machine, shows
// today's per-shift production + today's totals + upto-date totals.
//
// SP: sp_Prodn_Comber_OverAll (CompanyCode, FromDate, ToDate)
...
const TITLE = 'COMBER PRODUCTION DAY REPORT - MACHINE WISE';
const FILE_NAME = 'ComberProduction_Summary';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'COMBER PRODUCTION DAY REPORT - SUMMARY',
...
export const comberProductionSummaryReport = (req, res) => {
  return runReport(req, res, {
    spName: 'sp_Prodn_Comber_OverAll',
    fileName: FILE_NAME,
    buildDocDefinition
  });
};
