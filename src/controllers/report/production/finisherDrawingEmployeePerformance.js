// Finisher Drawing Employee Performance report.
// Mirrors 05rptFinisherDrawingEmployeePerformance.rdlc — one row per employee/date
// with production, efficiency, utilisation, index and grade; grouped by employee.
//
// SP: sp_Prodn_FinisherDrawing_EmployeePerformance (CompanyCode, FromDate, ToDate)
...
const TITLE = 'FINISHER DRAWING EMPLOYEE PERFORMANCES REPORT';
const FILE_NAME = 'FinisherDrawingProduction_EmployeePerformance';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const sorted = [...rows].sort((a, b) => {
    ...
    return new Date(a.FinisherDate) - new Date(b.FinisherDate);
  });
  ...
  for (const r of sorted) {
    ...
    body.push([
      cell(ddmmyyyy(r.FinisherDate), 'center'),
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'FINISHER DRAWING EMPLOYEE PERFORMANCE - SUMMARY',
...
export const finisherDrawingEmployeePerformanceReport = (req, res) => {
  return runReport(req, res, {
    spName: 'sp_Prodn_FinisherDrawing_EmployeePerformance',
    fileName: FILE_NAME,
    buildDocDefinition
  });
};
