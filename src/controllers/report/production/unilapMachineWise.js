// UniLap Machine Wise (Upto-Date Detail) report.
// Mirrors rptUnilapMachineWiseDetails_report.rdlc — per-machine view of upto-date
// Production / Efficiency / Utilization broken out by shift, plus upto-date totals.
// Differs from Drawing: production values are formatted as integers (no decimals)
// while Eff/Util/Index stay at 2 decimals — matches the RDLC's "#0" format spec.
//
// SP: sp_Prodn_Unilap_OverAll (CompanyCode, FromDate, ToDate)
...
// 19 columns: M/C, Mixing, Spd + 16 metric cols.
const WIDTHS = [
...
const TITLE = 'UNILAP MACHINE WISE UPDATE PRODUCTION';
const FILE_NAME = 'UnilapProduction_MachineWise';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  body.push([
    ...
    { text: fmt(sP1, 0), alignment: 'right', ...gStyle },
    { text: fmt(sP2, 0), alignment: 'right', ...gStyle },
    { text: fmt(sP3, 0), alignment: 'right', ...gStyle },
    { text: fmt(sPTot, 0), alignment: 'right', ...gStyle },
    ...
    { text: fmt(sUpProdn, 0), alignment: 'right', ...gStyle },
...
export const unilapMachineWiseReport = (req, res) => {
  return runReport(req, res, {
    spName: 'sp_Prodn_Unilap_OverAll',
    fileName: FILE_NAME,
    buildDocDefinition
  });
};
