// Spinning Stoppage Wise report.
// Mirrors 05rptSpinningProductionStoppage.rdlc — one row per stoppage reason, with
// HRS and % columns for each shift, plus on-date and upto-date totals.
//
// SP: sp_Prodn_Spinning_Stoppage (CompanyCode, FromDate, ToDate)
...
const TITLE = 'SPINNING STOPPAGE REPORT';
const FILE_NAME = 'SpinningProduction_StoppageWise';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'SPINNING STOPPAGE - SUMMARY',
...
export const spinningStoppageReport = (req, res) => {
  return runReport(req, res, {
    spName: 'sp_Prodn_Spinning_Stoppage',
    fileName: FILE_NAME,
    buildDocDefinition
  });
};
