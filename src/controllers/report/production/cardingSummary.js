// Carding Production Summary (Date Wise) report.
// Mirrors rptCardingProductionSheetDateWise.rdlc — one row per machine, shows
// today's per-shift production + today's totals + upto-date totals.
//
// SP: sp_Prodn_Carding_OverAll (CompanyCode, FromDate, ToDate)
...
const TITLE = 'CARDING PRODUCTION DAY REPORT - MACHINE WISE';
const FILE_NAME = 'CardingProduction_Summary';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'CARDING PRODUCTION DAY REPORT - SUMMARY',
...
export const cardingProductionSummaryReport = (req, res) => {
  return runReport(req, res, {
    spName: 'sp_Prodn_Carding_OverAll',
    fileName: FILE_NAME,
    buildDocDefinition
  });
};
