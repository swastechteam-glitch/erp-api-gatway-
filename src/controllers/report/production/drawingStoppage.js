// Breaker Drawing Stoppage Wise report.
// Mirrors rptDrawingProduction_Stoppage.rdlc — one row per stoppage reason, with
// HRS and % columns for each shift, plus on-date and upto-date totals.
//
// SP: sp_Prodn_Drawing_Stoppage (CompanyCode, FromDate, ToDate)
...
const TITLE = 'BREAKER DRAWING STOPPAGE REPORT';
const FILE_NAME = 'DrawingProduction_StoppageWise';
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  const summary = buildGroupSummaryPage({
    ...
    title: 'BREAKER DRAWING STOPPAGE - SUMMARY',
...
export const drawingStoppageReport = (req, res) => {
  return runReport(req, res, {
    spName: 'sp_Prodn_Drawing_Stoppage',
    fileName: FILE_NAME,
    buildDocDefinition
  });
};
