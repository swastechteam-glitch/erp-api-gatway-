// Comber Production Shift Wise Details report.
// Mirrors 01rptComberProductionShiftWise.rdlc — detail rows grouped per shift,
// each shift block headed by date / shift / supervisor (SIC) / maistry (MON),
// with a per-shift total row.
//
// SP: sp_Prodn_ComberProdnDetails_GetAll (CompanyCode, FromDate, ToDate)
...
// 18 columns.
const WIDTHS = [28, 52, 40, 40, 40, 38, 44, 44, 36, 36, 36, 40, 40, 38, 38, 36, 44, 40];

const TITLE = 'COMBER PRODUCTION - SHIFT WISE DETAILS';
const FILE_NAME = 'ComberProduction_ShiftWise';

const HEADERS = [
  'M/C No', 'Emp ID', 'Tgt Hank', 'Act Hank', 'Wrk Min', 'Speed',
  'Tgt Prdn', 'Act Prdn', 'Stop', 'Eff %', 'UT %', 'Energy',
  'UKG', 'Slv Brk', 'Crl Brk', 'DL Brk', 'Waste Kg', 'Waste %'
];
...
function buildShiftTable(shiftRows) {
  ...
  let sEnergy = 0, sUkg = 0, sSlv = 0, sCrl = 0, sDl = 0, sWasteKg = 0, sWastePer = 0;
  let n = 0;
  ...
  for (const r of shiftRows) {
    ...
    sEnergy += energy; sUkg += ukg; sSlv += slv; sCrl += crl; sDl += dl;
...
function buildDocDefinition({ rows, companyName, companyLogo, fromDate, toDate }) {
  ...
  for (const shiftRows of groups.values()) {
    ...
    const dateTxt = ddmmyyyy(head.CBRProdnDate);
...
export const comberShiftWiseReport = (req, res) => {
  return runReport(req, res, {
    spName: 'sp_Prodn_ComberProdnDetails_GetAll',
    fileName: FILE_NAME,
    buildDocDefinition
  });
};
