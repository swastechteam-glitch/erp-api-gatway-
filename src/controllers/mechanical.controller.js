...
// Gateway controller for "mechanical": forwards to the "core" service via axiosRequest.
export const mechanical = async (req, res, next) => {
  try {
    const r = await axiosRequest('core', {
      method: req.method,
      // req.originalUrl = the exact path the browser called, e.g.
      // "/api/v1/mechanical/...?year=2026". We forward it unchanged so the
